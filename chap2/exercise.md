...
> Given a training set $\mathcal{T}$ and a test point $x_0$, we want to know the expected prediction error of our linear model in theory. This error is essentially the mean squared error first averged among the training set, then conditioned at $x_0$ w.r.t. $y_0$,
>
> $$\mathrm{EPE}(x_0) = \mathrm{E}_{y_0|x_0}\left[ \mathrm{E}_\mathcal{T} \left[ (y_0 - \hat{y}_0)^2\right]\right].$$
>
> The inner expectation w.r.t. the training $\mathcal{T}$ has no influence on $y_0$, i.e., $\mathrm{E}\left[y_0\right] = y_0$, thus
>
...
> Further,
>
> $$\begin{aligned}\mathrm{E}_\mathcal{T}\left[\hat{y_0}^2\right] & = \mathrm{E}_\mathcal{T}\left[(\hat{y}_0 - \mathrm{E}_\mathcal{T}[\hat{y}_0] + \mathrm{E}_\mathcal{T}[\hat{y}_0])^2\right] \\ \text{(crossed term is 0)} & = \mathrm{E}_\mathcal{T}[(\hat{y}_0 - \mathrm{E}_\mathcal{T}[\hat{y}_0])^2] + \mathrm{E}_\mathcal{T}[\hat{y}_0]^2 \\ & = \mathrm{Var}_\mathcal{T}\left[\hat{y}_0 \right] + \mathrm{E}_\mathcal{T}[\hat{y}_0]^2 \end{aligned}$$
>
> Similarly,
>
> $$\mathrm{E}_\mathcal{y0|x_0}\left[y_0^2\right] = \mathrm{Var}\left[y_0\right] + \mathrm{E}_{y_0|x_0}[y_0]^2.$$
>
> Therefore,
>
> $$\begin{aligned}\mathrm{EPE}(x_0) & = \mathrm{Var}\left[y_0\right] + \mathrm{E}_{y_0|x_0}[y_0]^2 - 2\; \mathrm{E}_{y_0|x_0}[y_0]\; \mathrm{E}_\mathcal{T}[\hat{y}_0] + \mathrm{E}_\mathcal{T}[\hat{y}_0]^2 + \mathrm{Var}_\mathcal{T}\left[\hat{y}_0 \right] \\ & =  \mathrm{Var}\left[y_0\right] + (\mathrm{E}_{y_0|x_0}[y_0] - \mathrm{E}_\mathcal{T}[\hat{y}_0])^2 + \mathrm{Var}_\mathcal{T}\left[\hat{y}_0 \right] \\ & = \mathrm{Var}\left[y_0\right] + \mathrm{Bias}[\hat{y}_0]^2 + \mathrm{Var}_\mathcal{T}\left[\hat{y}_0 \right] \end{aligned}$$
>
> The variance of $y_0$ is caused by the randomness of $\epsilon$, and thus $\mathrm{Var}[y_0]=\sigma^2$. The linear regression model does not introduce any variance, and thus $\mathrm{Var}_\mathcal{T}[\hat{y}_0] = 0$. Moreover, $\mathrm{E}_{y_0|x_0}[y_0] = x_0^T\beta$ and $\hat{y}_0 = x_0^T\beta + x_0^T (\bm{X}^T \bm{X})^{-1} \bm{X}^T \epsilon$, so
>
> $$\mathrm{Bias}[\hat{y}_0] = \mathrm{E}_\mathcal{T}\left[x_0^T (\bm{X}^T \bm{X})^{-1} \bm{X}^T \epsilon\right]$$
>
